...
const LoginChoice = () => {
  ...
  const handleLoginChoice = (path) => {
    showToast(`Redirecting to ${path.split('/')[2].toUpperCase()} login`, 'info');
    navigate(path);
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        ...
        <div className="space-y-4">
          <button
            onClick={() => handleLoginChoice('/login/sms')}
          ...
          <button
            onClick={() => handleLoginChoice('/login/whatsapp')}
          ...
          <button
            onClick={() => handleLoginChoice('/login/email')}
            className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-4 px-6 rounded-xl font-semibold hover:from-purple-700 hover:to-pink-700 transition-all transform hover:scale-[1.02] flex items-center justify-center gap-3"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
