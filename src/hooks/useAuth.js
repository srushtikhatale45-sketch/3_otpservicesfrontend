...
export const useVerifySMSOTP = () => {
  ...
  return useMutation({
    mutationFn: ({ phoneNumber, otpCode, name }) => 
      authService.verifySMSOTP(phoneNumber, otpCode, name),
    onSuccess: (data) => { 
      if (data.verified) queryClient.invalidateQueries({ queryKey: authKeys.check() });
    }
  });
};
...
export const useVerifyWhatsAppOTP = () => {
  ...
  return useMutation({
    mutationFn: ({ phoneNumber, otpCode, name }) => 
      authService.verifyWhatsAppOTP(phoneNumber, otpCode, name),
    onSuccess: (data) => { 
      if (data.verified) queryClient.invalidateQueries({ queryKey: authKeys.check() });
    }
  });
...
export const useSendEmailOTP = () => {
  return useMutation({ mutationFn: authService.sendEmailOTP });
};

export const useVerifyEmailOTP = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ email, otpCode, name }) => 
      authService.verifyEmailOTP(email, otpCode, name),
    onSuccess: (data) => { 
      if (data.verified) queryClient.invalidateQueries({ queryKey: authKeys.check() });
    }
  });
...
export const useResendEmailOTP = () => {
  return useMutation({ mutationFn: authService.resendEmailOTP });
};
